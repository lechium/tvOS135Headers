//
//     Generated by classdumpios 1.0.1 (64 bit) (iOS port by DreamDevLost)(Debug version compiled Jun 10 2020 10:03:13).
//
//  Copyright (C) 1997-2019 Steve Nygard.
//

#import "CKKSCKRecordHolder.h"

@class CKKSWrappedAESSIVKey, NSData, NSNumber, NSString;

__attribute__((visibility("hidden")))
@interface CKKSItem : CKKSCKRecordHolder
{
    NSString *_uuid;	// 8 = 0x8
    NSString *_parentKeyUUID;	// 16 = 0x10
    NSData *_encitem;	// 24 = 0x18
    CKKSWrappedAESSIVKey *_wrappedkey;	// 32 = 0x20
    unsigned long long _generationCount;	// 40 = 0x28
    unsigned long long _encver;	// 48 = 0x30
    NSNumber *_plaintextPCSServiceIdentifier;	// 56 = 0x38
    NSData *_plaintextPCSPublicKey;	// 64 = 0x40
    NSData *_plaintextPCSPublicIdentity;	// 72 = 0x48
}

+ (id)fromDatabaseRow:(id)arg1;	// IMP=0x0000000100054374
+ (id)sqlColumns;	// IMP=0x0000000100054284
+ (void)setOSVersionInRecord:(id)arg1;	// IMP=0x0000000100054228
- (void).cxx_destruct;	// IMP=0x0000000100054184
@property(retain) NSData *plaintextPCSPublicIdentity; // @synthesize plaintextPCSPublicIdentity=_plaintextPCSPublicIdentity;
@property(retain) NSData *plaintextPCSPublicKey; // @synthesize plaintextPCSPublicKey=_plaintextPCSPublicKey;
@property(retain) NSNumber *plaintextPCSServiceIdentifier; // @synthesize plaintextPCSServiceIdentifier=_plaintextPCSServiceIdentifier;
@property unsigned long long encver; // @synthesize encver=_encver;
@property unsigned long long generationCount; // @synthesize generationCount=_generationCount;
@property(copy) CKKSWrappedAESSIVKey *wrappedkey; // @synthesize wrappedkey=_wrappedkey;
@property(copy) NSData *encitem; // @synthesize encitem=_encitem;
@property(copy) NSString *parentKeyUUID; // @synthesize parentKeyUUID=_parentKeyUUID;
@property(copy) NSString *uuid; // @synthesize uuid=_uuid;
- (id)sqlValues;	// IMP=0x0000000100053b80
- (id)whereClauseToFindSelf;	// IMP=0x0000000100053a78
@property(retain, getter=base64Item, setter=setBase64Item:) NSString *base64encitem;
- (id)copyWithZone:(struct _NSZone *)arg1;	// IMP=0x00000001000538f8
- (id)debugDescription;	// IMP=0x0000000100053854
- (id)description;	// IMP=0x00000001000537b4
- (id)makeAuthenticatedDataDictionaryUpdatingCKKSItemEncVer2:(id)arg1;	// IMP=0x00000001000530ec
- (id)makeAuthenticatedDataDictionaryUpdatingCKKSItemEncVer1;	// IMP=0x0000000100052ee0
- (id)makeAuthenticatedDataDictionaryUpdatingCKKSItem:(id)arg1 encryptionVersion:(unsigned long long)arg2;	// IMP=0x0000000100052de0
- (_Bool)matchesCKRecord:(id)arg1;	// IMP=0x000000010005264c
- (id)updateCKRecord:(id)arg1 zoneID:(id)arg2;	// IMP=0x0000000100052244
- (void)setFromCKRecord:(id)arg1;	// IMP=0x0000000100051e50
- (id)CKRecordName;	// IMP=0x0000000100051e44
- (_Bool)isEqual:(id)arg1;	// IMP=0x0000000100051a80
- (id)initWithUUID:(id)arg1 parentKeyUUID:(id)arg2 zoneID:(id)arg3 encodedCKRecord:(id)arg4 encItem:(id)arg5 wrappedkey:(id)arg6 generationCount:(unsigned long long)arg7 encver:(unsigned long long)arg8 plaintextPCSServiceIdentifier:(id)arg9 plaintextPCSPublicKey:(id)arg10 plaintextPCSPublicIdentity:(id)arg11;	// IMP=0x0000000100051874
- (id)initWithUUID:(id)arg1 parentKeyUUID:(id)arg2 zoneID:(id)arg3 encodedCKRecord:(id)arg4 encItem:(id)arg5 wrappedkey:(id)arg6 generationCount:(unsigned long long)arg7 encver:(unsigned long long)arg8;	// IMP=0x0000000100051778
- (id)initWithUUID:(id)arg1 parentKeyUUID:(id)arg2 zoneID:(id)arg3 encItem:(id)arg4 wrappedkey:(id)arg5 generationCount:(unsigned long long)arg6 encver:(unsigned long long)arg7;	// IMP=0x00000001000516a0
- (id)initWithUUID:(id)arg1 parentKeyUUID:(id)arg2 zoneID:(id)arg3;	// IMP=0x0000000100051610
- (id)initCopyingCKKSItem:(id)arg1;	// IMP=0x00000001000513a4
- (id)initWithCKRecord:(id)arg1;	// IMP=0x0000000100051370

@end

